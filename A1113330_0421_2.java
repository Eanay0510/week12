...
public class A1113330_0421_2{
    public static void main(String[] args){
        
        Scanner sc = new Scanner(System.in);

        String checkdate1="[0-9]{4}/([0][0-9]||[1][1-2])/([0-2][0-9]||[3][0-1])";
        String checkdate2="([0][0-9]||[1][1-2])/([0-2][0-9]||[3][0-1])/[0-9]{4}";

        System.out.println("請依照格式輸入日期(YYYY/MM/DD or MM/DD/YYYY)：");
        String date=sc.next();

        if( !(date.matches(checkdate1) || date.matches(checkdate2))){
            System.out.println("請輸入正確的日期格式(YYYY/MM/DD or MM/DD/YYYY)：");
            date=sc.next();
        }

        if(date.matches(checkdate1)){
            System.out.println(date.substring(0,4)+"年"+date.substring(5,7)+"月"+date.substring(8,10)+"日");
        }
        else if(date.matches(checkdate2)){
